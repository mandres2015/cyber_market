...
ipcMain.on('getProducts', (e, params) => {
    ...
    const client = new Client(credentials)
    const query = `SELECT p.id, p.code, p.name, p.has_iva as iva, p.has_ice as ice, b.name as brand, m.name as measure 
                    FROM product p 
...
ipcMain.on('saveProduct', (e, params) => {
    ...
    const query = `INSERT INTO public.product(code, name, has_iva, has_ice, measure_id, brand_id, category_id)
                    VALUES ($1, $2, $3, $4, $5, $6, $7);`

    params = cleanParams(params)
    const client = new Client(credentials)
    client.connect()
    client.query(query, [params.code, params.name, params.hasIVA, params.hasICE, params.measure, params.brand, params.category], (err, res) => {
        if (err) {
            response.status = -1
            response.data = err
